...
LATEST_RELEASE_URL=$(curl -s https://api.github.com/repos/Bachar-official/dart_cmd_bot/releases/latest | grep "browser_download_url.*dart_cmd_bot_linux" | cut -d '"' -f 4)
wget -O dart_cmd_bot_linux "$LATEST_RELEASE_URL"
chmod +x dart_cmd_bot_linux
...
[Service]
ExecStart=$(pwd)/dart_cmd_bot_linux
Restart=always
